...
| `oidc_issuer` | string | OIDC Issuer |
| `oidc_subject` | string | OIDC Subject |
| `wechat_id` | string | 微信用户 ID |
| `status` | string | 用户状态：`active`、`ban` |
| `last_login_at` | string | 最近登录时间 |
| `extra` | json | 扩展信息，第三方资料按平台命名空间保存，如 `linuxDo`、`oidc`；接入 New API 时会在服务端保存 `newApi` 授权信息，接口响应会隐藏该字段 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `availableModels` | string[] | 系统可用模型列表；用户侧本地直连和云端渠道优先使用 New API 按当前登录用户返回的模型列表 |
...
| `allowCustomChannel` | bool | 是否允许用户自定义渠道，默认允许，关闭后前端只提供云端渠道模式 |
...
`auth.passwordLogin` 当前字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `enabled` | bool | 是否在前端登录页显示账号密码和注册表单，默认显示；仅影响前端展示，不关闭后端注册能力或 OIDC 首次登录创建用户 |

`auth.oidc` 当前字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `enabled` | bool | 是否开启通用 OIDC 登录，可用于接入 Logto |
| `name` | string | OIDC 登录按钮展示名称，默认 `Logto` |
...
| `scope` | string | OIDC scope，默认 `openid profile email`；配置 New API 后会自动追加 New API ecosystem scope 和 `offline_access` |

用户侧默认使用本地直连：后端会读取 `.env` 中的 `NEW_API_BASE_URL`，用用户 OIDC 登录时取得的 Logto resource token 调用 New API `/api/ecosystem/models` 和 `/api/ecosystem/tokens`，再把当前用户可用模型、令牌名称、`baseUrl` 和 `apiKey` 返回给前端，前端默认选择第一个令牌并由浏览器直接请求 New API。用户手动切到云端渠道时，后端仍会使用 New API 返回的用户令牌转发 `/api/v1/*` 请求。后台私有渠道保留给后台管理和兼容配置使用。
