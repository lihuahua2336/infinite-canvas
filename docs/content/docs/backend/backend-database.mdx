...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `availableModels` | string[] | 系统可用模型列表；用户侧云端渠道优先使用 New API 按当前登录用户返回的模型列表 |
...
用户侧云端渠道请求模型时，后端会读取 `.env` 中的 `NEW_API_BASE_URL`，用用户 OIDC 登录时取得的 Logto resource token 调用 New API `/api/ecosystem/models` 和 `/api/ecosystem/tokens`，再使用 New API 返回的用户令牌转发 `/api/v1/*` 请求。后台私有渠道仍保留给后台管理和兼容配置使用。
