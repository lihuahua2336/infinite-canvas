...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `availableModels` | string[] | 系统可用模型；保存设置时会自动合并所有已启用私有渠道的模型。用户侧云端渠道会优先读取 New API 返回的当前用户可用模型 |
...
| 模式 | 说明 |
| --- | --- |
| 云端渠道 | 默认模式。登录用户通过 Logto 获取 New API ecosystem 授权，前端读取后端 `/api/new-api/config` 返回的 New API 模型列表；实际 `/api/v1/*` 请求由后端使用 New API 返回的用户令牌转发 |
...
| `NEW_API_PUBLIC_URL` | 前端跳转到 New API 登录和令牌配置页的浏览器地址 |
