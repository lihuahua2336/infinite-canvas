...
async function persistCanvasImageTask(task: CanvasImageTask) {
    ...
    const stored = await Promise.all(urls.map((url, index) => {
        const existing = task.imageStorage?.find((image) => image?.url === url);
        if (existing?.storageKey) return existing;
        if (task.storageKeys?.[index] || (index === 0 && task.storageKey)) return null;
        return uploadImage(url, { localOnly: true }).catch(() => null);
    }));
    ...
    return {
        ...
        storageKey: stored[0]?.storageKey || task.storageKey || "",
        storageKeys: stored.map((image, index) => image?.storageKey || task.storageKeys?.[index] || (index === 0 ? task.storageKey : "") || ""),
        imageStorage: stored,
    };
}
    ...
    const childNodes = urls.map((url, index): CanvasNodeData => {
        ...
        return {
            ...
            metadata: {
                ...
                status: NODE_STATUS_SUCCESS,
                progress: 100,
                storageKey: task.storageKeys?.[index] || stored?.storageKey || "",
