...
+ [修复] OpenAI 兼容图生图流式返回支持包装后的图片事件，避免接口实际返回图片但前端误报“接口没有返回图片”。

## v1.0.4 - 2026-06-29

+ [发布] 版本检查源切换到当前发布仓库，确保应用内最新版本读取 `lihuahua2336/infinite-canvas`。
+ [发布] 移除不再需要的文档站镜像发布工作流，后续 tag 发布只触发主应用镜像更新。
